/**
 * a-hole - Black hole visual component
 * Inline easeInExpo (replaces easing-utils import)
 */
...
class AHole extends HTMLElement {
  ...
  initParticle(start = false) {
    const ex = this.particleArea.sx + this.particleArea.sw * Math.random();
    const sx = this.particleArea.ex + this.particleArea.ew * Math.random();
    const dx = ex - sx;
  ...
  moveDiscs() {
    this.discs.forEach((disc) => {
      disc.p = (disc.p - 0.001 + 1) % 1; // reverse: discs expand from center
      this.tweenDisc(disc);
    });
  }
  ...
  moveParticles() {
    this.particles.forEach((particle) => {
      particle.p = particle.y / this.particleArea.h;
      particle.x = particle.sx + particle.dx * (1 - particle.p);
      particle.y += particle.vy; // reverse: particles fall into hole from above

      if (particle.y > this.particleArea.h) {
